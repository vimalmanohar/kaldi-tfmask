...
if [ ! -f data/train_orig/.mfcc.done ]; then
  cp -rT data/train data/train_orig

  bash -c 'rm data/train_orig/{feats.scp,cmvn.scp}; true'
  bash -c 'rm -rf data/train_orig/split*; true'

  if $use_pitch; then
    steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj $train_nj data/train_orig exp/make_mfcc_pitch/train_orig mfcc
  else
    steps/make_mfcc.sh --cmd "$train_cmd" --nj $train_nj data/train_orig exp/make_mfcc/train_orig mfcc
  fi
  utils/fix_data_dir.sh data/train_orig
  steps/compute_cmvn_stats.sh data/train_orig exp/make_mfcc/train_orig mfcc
  utils/fix_data_dir.sh data/train_orig
  touch data/train_orig/.mfcc.done
fi

if [ ! -f data/train/.mfcc.done ]; then
  mkdir -p data_fbank
  cp -rT data/train_orig data_fbank/train
  bash -c "rm data_fbank/train/{feats.scp,cmvn.scp}; rm -rf data_fbank/train/split*; true"
  
  steps/make_fbank.sh --cmd "$train_cmd" --nj $train_nj data_fbank/train exp/make_fbank/train fbank

  local/make_masked_fbank.sh --cmd "$train_cmd" --nj $train_nj data_fbank/train data_fbank/train_masked exp/irm_nnet exp/make_masked_fbank/train irm fbank
  
  if $use_pitch; then
    local/make_mfcc_from_fbank.sh --cmd "$train_cmd" --nj $train_nj data_fbank/train_masked data/train_tmp exp/make_mfcc/train mfcc
    
    cp -rT data/train_orig data/train_pitch_tmp
    bash -c "rm data/train_pitch_tmp/{cmvn.scp,feats.scp}; true"
    bash -c "rm -rf data/train_pitch_tmp/split*; true"

    steps/make_pitch_kaldi.sh --cmd "$train_cmd" --nj $train_nj data/train_pitch_tmp exp/make_mfcc/train_pitch_tmp mfcc
    steps/append_feats.sh --cmd "$train_cmd" --nj $train_nj data/train_tmp data/train_pitch_tmp data/train exp/make_mfcc/train_append_pitch mfcc
  else
    local/make_mfcc_from_fbank.sh --cmd "$train_cmd" --nj $train_nj data_fbank/train_masked data/train exp/make_mfcc/train mfcc
  fi
  ...
  rm -rf data/train{,_pitch}_tmp
  bash -c "rm -rf data/train/split*; true"
