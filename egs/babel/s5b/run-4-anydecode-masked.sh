...
function make_fbank_mfcc {
  ...
  if $use_pitch; then
    local/make_mfcc_from_fbank.sh --cmd "$decode_cmd" --nj $my_nj $src ${target}_tmp $logdir $output
    cp -rT $target ${target}_pitch_tmp
    bash -c "rm ${target}_pitch_tmp/{cmvn.scp,feats.scp}; true"
    bash -c "rm -rf ${target}_pitch_tmp/split*; true"
    
    steps/make_pitch_kaldi.sh --cmd "$decode_cmd" --nj $my_nj ${target}_pitch_tmp ${logdir}_pitch_tmp $output
    steps/append_feats.sh --cmd "$decode_cmd" --nj $my_nj ${target}_tmp ${target}_pitch_tmp ${target} ${logdir}_append_pitch $output

  else
    local/make_mfcc_from_fbank.sh --cmd "$decode_cmd" --nj $my_nj $src $target $logdir $output
  fi
  ...
  rm -rf ${target}{,_pitch}_tmp
  bash -c "rm -rf ${target}/split*; true"
}
...
if [ ! -f  $dataset_dir/.done ] ; then
  ...
  if [ ! -f ${dataset_dir}_orig/.mfcc.done ]; then
    cp -rT ${dataset_dir} ${dataset_dir}_orig

    echo ---------------------------------------------------------------------
    echo "Preparing ${dataset_kind} parametrization files in ${dataset_dir} on" `date`
    echo ---------------------------------------------------------------------
    make_mfcc ${dataset_dir}_orig exp/make_mfcc/${dataset_id} mfcc
    touch ${dataset_dir}_orig/.mfcc.done
  fi
  ...
  if [ ! -f ${dataset_dir}/.mfcc.done ]; then
    ...
    mkdir -p data_fbank
    cp -rT ${dataset_dir}_orig data_fbank/${dataset_id}
    bash -c "rm data_fbank/${dataset_id}/{feats.scp,cmvn.scp}; rm -rf data_fbank/${dataset_id}/split*; true"

    steps/make_fbank.sh --cmd "$decode_cmd" --nj $my_nj data_fbank/${dataset_id} exp/make_fbank/${dataset_id} fbank
  
    local/make_masked_fbank.sh --cmd "$decode_cmd" --nj $my_nj data_fbank/${dataset_id} data_fbank/${dataset_id}_masked exp/irm_nnet exp/make_masked_fbank/${dataset_id} irm fbank

    make_fbank_mfcc data_fbank/${dataset_id}_masked ${dataset_dir} exp/make_mfcc/${dataset_id} mfcc
