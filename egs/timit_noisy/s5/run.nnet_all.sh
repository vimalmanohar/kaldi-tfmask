...
for trainid in clean random; do
  ...
  if [ ! -f exp/tri4_nnet_${trainid}/final.mdl ]; then
    ...
    for t in data/dev_clean data/test_clean; do
      if [ ! -f exp/tri4_nnet_${trainid}/decode_${t##*/}/.done ]; then
        local/run_nnet.decode.sh $t exp/tri3/graph exp/tri4_nnet_${trainid}
      fi
    done

    while read noise_type <&3; do
      while read snr <&4; do
        for t in data/dev_noisy_${noise_type}_snr_${snr} data/test_noisy_${noise_type}_snr_${snr}; do 
          #steps/compute_cmvn_stats.sh $t exp/make_fbank_mfcc/`basename $t` fbank_mfcc
          #utils/fix_data_dir.sh $t
          if [ ! -f exp/tri4_nnet_${trainid}/decode_${t##*/}/.done ]; then
            local/run_nnet.decode.sh $t exp/tri3/graph exp/tri4_nnet_${trainid} || exit 1
          fi
        done
      done 4< snr.list
