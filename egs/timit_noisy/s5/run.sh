...
function make_fbank {
  ...
  set -e
  steps/make_fbank.sh --cmd "$train_cmd" --nj $nj $dest_dir $exp/make_fbank/$x $fbankdir
  steps/compute_cmvn_stats.sh --fake $dest_dir $exp/make_fbank/$x $fbankdir
...
function make_fbank_mfcc {
  ...
  local/make_mfcc_from_fbank.sh --cmd "$train_cmd" --nj $nj $src_dir $dest_dir $exp/make_fbank_mfcc/$x $mfccdir
  steps/compute_cmvn_stats.sh $dest_dir $exp/make_fbank_mfcc/$x $mfccdir
  utils/fix_data_dir.sh $dest_dir
}
...
function make_masked_fbank {
  ...
  local/make_masked_fbank.sh --cmd "$train_cmd" --nj $nj \
    $src_dir $dest_dir $irm_nnet_dir $exp/make_masked_fbank/$x $irm_dir $fbankdir
  steps/compute_cmvn_stats.sh --fake $dest_dir $exp/make_masked_fbank/$x $fbankdir
  utils/fix_data_dir.sh $dest_dir
}
...
function make_concat_mfcc {
  ...
  local/make_concat_feats.sh --cmd "$train_cmd" --nj $nj \
    $dest_dir $feat1_dir $feat2_dir $exp/make_fbank_mfcc/$x $mfccdir
  steps/compute_cmvn_stats.sh $dest_dir $exp/make_fbank_mfcc/$x $mfccdir
  utils/fix_data_dir.sh $dest_dir
}
