...
while read noise_type <&3; do
  while read snr <&4; do
    ...
    if [ ! -f data_fbank/train_noisy_${noise_type}_snr_$snr/.irm_targets.done ]; then
      local/make_irm_targets.sh data_fbank/train_noisy_${noise_type}_snr_$snr \
        data_fbank/train_clean $exp/make_irm_targets/train_noisy_${noise_type}_snr_$snr irm_targets || exit 1
      touch data_fbank/train_noisy_${noise_type}_snr_$snr/.irm_targets.done
    fi
