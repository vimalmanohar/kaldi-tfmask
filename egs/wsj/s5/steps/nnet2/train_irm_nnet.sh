...
feat_dim=`feat-to-dim "scp:head -1 $data/feats.scp |" ark,t:- | cut -d ' ' -f 2` || exit 1
...
if [ $stage -le -2 ]; then
  ...
  if [ ! -z "$spk_vecs_dir" ]; then
    ...
    spliced_dim=$[$feat_dim * (1+2*$splice_width) + $spk_vec_dim]
  else
    spk_vec_dim=0
    ext_feat_dim=$feat_dim
    spliced_dim=$[$feat_dim * (1+2*$splice_width)]
  fi
...
AffineComponent input-dim=$spliced_dim output-dim=$pnorm_input_dim learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
PnormComponent input-dim=$pnorm_input_dim output-dim=$pnorm_output_dim p=$p
NormalizeComponent dim=$pnorm_output_dim
AffineComponent input-dim=$pnorm_output_dim output-dim=$irm_fbank_dim learning-rate=$initial_learning_rate param-stddev=0 bias-stddev=0
SigmoidComponent dim=$irm_fbank_dim
EOF
...
AffineComponent input-dim=$pnorm_output_dim output-dim=$pnorm_input_dim learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
PnormComponent input-dim=$pnorm_input_dim output-dim=$pnorm_output_dim p=$p
NormalizeComponent dim=$pnorm_output_dim
AffineComponent input-dim=$pnorm_output_dim output-dim=$irm_fbank_dim learning-rate=$initial_learning_rate param-stddev=0 bias-stddev=0
SigmoidComponent dim=$irm_fbank_dim
