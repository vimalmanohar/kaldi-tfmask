...
# scripts, extracts the training examples used to train the neural net to predict Ideal Ratio Mask (and also
...
if [ $# != 3 ]; then
  echo "Usage: steps/nnet2/get_irm_egs.sh [opts] <data> <irm_target_scp> <exp-dir>"
  echo " e.g.: steps/nnet2/get_irm_egs.sh data/train_noisy data/train_noisy/irm.scp exp/irm_nnet"
  echo ""
...
cmvn_opts=`cat $transform_dir/cmvn_opts 2>/dev/null`
cp $transform_dir/cmvn_opts $dir 2>/dev/null
...
case $feat_type in
  ...
  lda) 
    ...
    cp $transform_dir/splice_opts $dir 2>/dev/null
