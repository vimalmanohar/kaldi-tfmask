...
template<typename Real>
__global__
static void _cuda_comp_obj_deriv_xent_sum(MatrixElement<Real> *x, int s, const Real* z, MatrixDim d, Real* z2, MatrixDim d2, Real* t) {
  int i = threadIdx.x;
  __shared__ Real tot_objf[CU1DBLOCK];
  __shared__ Real tot_weight[CU1DBLOCK];
  ...
  for(int j = loop_start; j< loop_end; j++) {
    ...
    tmp_tot_objf += (weight * this_prob + (1-weight) * log(n_this_prob));

    *(z2 + m * d2.stride + label ) += (weight - this_prob) / (this_prob * (n_this_prob));
  }
  tot_objf[i] = tmp_tot_objf;
  tot_weight[i] = tmp_weight_sum;
  __syncthreads();
...
void cudaF_comp_obj_deriv_xent_sum(dim3 Gr, dim3 Bl, MatrixElement<float>* x, int s, const float* z, MatrixDim d, float* z2, MatrixDim d2, float* t) {
  _cuda_comp_obj_deriv_xent_sum<<<Gr,Bl>>>(x,s,z,d,z2,d2,t);
}
...
void cudaD_comp_obj_deriv_xent_sum(dim3 Gr,dim3 Bl, MatrixElement<double>* x, int s, const double* z, MatrixDim d, double* z2, MatrixDim d2, double* t) {
  _cuda_comp_obj_deriv_xent_sum<<<Gr,Bl>>>(x,s,z,d,z2,d2,t);
}
